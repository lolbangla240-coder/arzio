...
(function(){
  ...
  function renderGrid(){
    ...
    if(!list.length){
      grid.innerHTML = '<div class="empty-state">No products listed yet.<br>Open <b>js/products.js</b> and add your first item.</div>';
      return;
    }
    ...
    grid.innerHTML = list.map((p, i) => `
      <div class="card">
        ...
        <div class="perf"></div>
        <div class="card-body">
          <div class="item-no">ITEM NO. ${String(i+1).padStart(3,'0')}</div>
          <h3>${escapeHtml(p.name)}</h3>
          ${p.origin ? `<div class="origin">ORIGIN — ${escapeHtml(p.origin.toUpperCase())}</div>` : ''}
          <p class="desc">${escapeHtml(p.desc || '')}</p>
          <div class="card-foot">
            <span class="price">${escapeHtml(p.price || '')}</span>
            <a class="order-btn" href="${FB_LINK}" target="_blank" rel="noopener">Order Now →</a>
          </div>
        </div>
      </div>
    `).join('');
